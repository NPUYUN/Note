...
git branch <new-branch>	新建分支（也可以用 git checkout -b <new-branch>，此时会将HEAD切换到new-branch）
...
## 推送（Push）
``` bash
git push 远程库链接/别名 分支名   推送远程库，分支名默认为HEAD
git push 远程库链接/别名 引用位置:分支名   指定位置推送远程库，当引用位置为空时，会删除分支名
```

## 拉取（Pull）
``` bash
git pull 远程库链接/别名 分支名   拉取远程库，为git fetch 与 git merge 的简写，即获取数据后合并到本地库，分支名默认为HEAD
git pull --rebase 远程库链接/别名 分支名   git fetch 与 git rebase 的简写
```
...
git fetch 远程库链接/别名 分支名  从远程库获取该分支名的数据（补全本地库缺失部分，并更新远程库在本地的分支指针），但是不修改本地文件，即不动本地的主分支
git fetch 远程库链接/别名 分支名:引用位置  从远程库获取该分支名的数据到引用位置（补全本地库缺失部分，并更新远程库在本地的分支指针），但是不修改本地文件，即不动本地的主分支，当分支名为空时，创建引用位置
```

# 追踪
```bash
远程库在本地具有 <库名>/main 的分支，反映了远程库当前的状态，随远程库更新而更新，默认被main跟踪
git checkout -b <new-branch> <库名>/main 新建分支并跟踪远程库分支
git branch -u <库名>/main 分支名  让该分支追踪远程库分支，如果此时HEAD指向该分支，则分支名可以省略
```