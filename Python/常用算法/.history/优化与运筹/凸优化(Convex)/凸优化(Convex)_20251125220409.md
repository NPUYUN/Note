...
- 这里为标准的流程，实际使用时可以参照**线性规划**中的写法，更为简洁。
...
### *半定规划
最大割问题是图论中的一个经典NP难问题。给定一个无向图G=(V, E)，以及边上的权重向量 w（wij​表示连接顶点i和j的边的权重）。
目标是将图的顶点集V划分为两个不相交的子集S和V\S，使得连接这两个子集的所有边的权重和最大


线性规划见**优化与运筹**的**线性规划**部分。
