...
核心函数为：
- `nx.shortest_path(G, source, target, weight='weight')` 计算从节点source到节点target的最短路径。
- `nx.shortest_path_length(G, source, target, weight='weight')` 计算从节点source到节点target的最短路径长度（总权重）。
其中，
- `G` 是图对象。
- `source` 是源节点。
- `target` 是目标节点。
- `weight` 是边的权重属性名，默认值为'weight'。
...
## 一个典型的最短路问题
已知有n个点，m条边的加权图，求从源点s到目标点t的最短路径，若不存在路径则输出-1。