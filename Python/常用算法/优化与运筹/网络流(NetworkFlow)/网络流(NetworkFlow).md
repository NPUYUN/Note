...
- `cost_of_flow (flowG, flow_dict, capacity='capacity', cost='cost')` 计算最小费用最大流的总费用

其中，
- `flowG` 是一个 NetworkX 图对象，包含了网络的结构和边的属性（如容量 `capacity` 和成本 `cost`）。
- `flow_dict` 是一个字典，包含了最大流问题的流量分配结果，键为顶点，值为一个字典，键为该顶点的邻居顶点，值为流量值。(即max_flow_min_cost的返回值)
...
- 计算最大流值为 `sum(flow_dict['s'].values())`。即源点 S 流出的流量总和。
- 输出各边流量分配详情，只显示有流量的边。

## 一个典型的最小费用最大流问题
给出一个包含 N个点（节点）和 M条有向边的网络图。图中有一个源点 S（产生流量）和一个汇点 T（接收流量）。每条边都有两个属性：最大流量（容量）和单位流量的费用。要求计算从源点 S到汇点 T的网络最大流，以及在达到最大流时的最小总费用
